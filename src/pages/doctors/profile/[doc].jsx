import Axios from "axios";
import React, { useState, useEffect } from "react";
import Head from "next/head";
import { useRouter } from "next/router";
import Image from "next/image";
import DoctorInfo from "../../../../components/DoctorInfo";
// process.env.API_TOKEN

const DoctorProfile = () => {  
  // console.log("token "+ process.env.API_TOKEN);
    const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..5flWhLIj0pP1bPfGrAKaV6h8NXsQM9siAB0fzJyYtAU"
    
    const axiosInstance = Axios.create({
        headers: {
          Authorization: `Bearer ${token}`,
        },
    });
    const router  = useRouter();
    const { doc } = router.query;
    const [myData, setData] = useState(null);
    const [myDataB, setDataB] = useState(null);
    const [myDataC, setDataC] = useState(null);
    const [myDataD, setDataD] = useState(null);
  
    useEffect(() => {
      const apiUrl = typeof window !== 'undefined' ? process.env.API_URL : '';
      if (doc) {
        axiosInstance
          .get(`${apiUrl}/profile/doctor/${doc}/info`)
          .then((res) => {
            setData(res.data);
          })
          .catch((err) => {
            console.log(res.err);
          });
  
        axiosInstance
          .get(`${apiUrl}/review/doctor/${doc}`)
          .then((res) => {
            setDataB(res.data);
          })
          .catch((err) => {
            console.log(res.err);
          });

        axiosInstance
          .get(`${apiUrl}/profile/${doc}/educations`)
          .then((res) => {
            setDataC(res.data);
          })
          .catch((err) => {
            console.log(res.err);
          });

        axiosInstance
          .get(`${apiUrl}/profile/${doc}/experiences`)
          .then((res) => {
            setDataD(res.data);
            console.log(res.data);
          })
          .catch((err) => {
            console.log(res.err);
          });
      }
    }, [doc]);

    // const objData = myDataC;
    // let allData = []
    // for (let value of Object.fromEntries(objData.educations)) {
    //   allData = Object.values(value);    
    // }
    // console.log(JSON.stringify(myDataD))
    // console.log('Education ' + allData)

  return (
    <>
<div>
  {myData && myDataB && myDataC && myDataD ? (
    <section className="w-full px-4 pt-6 pb-6 md:pt-28 md:pb-12 doctors-list bg-pattern">
      <div className="container mx-auto max-w-screen-xl">{/* <!-- container start --> */}
        <DoctorInfo doctor={myData.doctor} review={myDataB.foundReviews} educations={myDataC.educations} experiences={myDataD.experiences} />
       
      </div>
    </section>           
  ) : (
    <p>Loading data...</p>
  )}
</div>
     </>
      )
  
}

export default DoctorProfile